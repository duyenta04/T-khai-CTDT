...
Feature: Đăng ký và thay đổi thông tin sử dụng chứng từ điện tử
  ...
  Rule: Xem và quản lý danh sách tờ khai
    ...
    @detail-view
    Scenario: Hiển thị đầy đủ thông tin khi xem chi tiết
    ...
    Scenario: Hiển thị chi tiết lý do từ chối khi tờ khai bị từ chối
      Given người nộp thuế đang xem chi tiết tờ khai
      And tờ khai có trạng thái "CQT không tiếp nhận"
      Then hệ thống hiển thị lý do từ chối từ cơ quan thuế

    @detail-view @rejection
    Scenario: Lý do từ chối bao gồm thông tin chi tiết
      Given người nộp thuế đang xem tờ khai bị từ chối
      When người nộp thuế xem lý do từ chối
      Then thông tin lý do từ chối bao gồm
        | Mã lỗi            |
        | Mô tả lỗi         |
        | Hướng dẫn khắc phục |
