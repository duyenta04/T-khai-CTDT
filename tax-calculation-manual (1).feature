...
Feature: Nhập thông tin thuế thu nhập cá nhân khấu trừ
  ...
  Rule: Xem trước chứng từ
    ...
    Scenario: Xem trước chứng từ trước khi phát hành
      ...
      Then hệ thống hiển thị bản xem trước chứng từ theo định dạng chính thức
      And hiển thị đầy đủ thông tin:
        | Phần                          | Nội dung hiển thị                    |
        | Tiêu đề                       | CHỨNG TỪ KHẤU TRỪ THUẾ TNCN          |
        | Mẫu số                        | 03/TNCN-CT/26E                       |
        | Thông tin người nộp thuế      | Họ tên, MST, CCCD, địa chỉ           |
        | Thông tin thu nhập            | Khoản thu nhập, kỳ tính thuế         |
        | Thông tin tính thuế           | Thu nhập chịu thuế, các khoản khấu trừ|
        | Kết quả                       | Thu nhập tính thuế, số thuế          |

    @preview @format
    Scenario: Xem trước chứng từ định dạng PDF
      Given kế toán đang xem trước chứng từ
      When kế toán chọn định dạng "PDF"
      Then chứng từ được hiển thị dạng PDF
      And có thể in hoặc tải về

    @preview @format
    Scenario: Xem trước chứng từ định dạng XML
      Given kế toán đang xem trước chứng từ
      When kế toán chọn định dạng "XML"
      Then chứng từ được hiển thị dạng XML
      And hiển thị cấu trúc dữ liệu sẽ gửi lên CQT

    @preview @validation
    Scenario: Phát hiện lỗi khi xem trước
      Given kế toán đã nhập thông tin thuế
      And có trường dữ liệu không hợp lệ
      When kế toán chọn "Xem trước"
      Then hệ thống hiển thị thông báo lỗi
      And không hiển thị bản xem trước
      And liệt kê các lỗi cần sửa

    @preview @edit
    Scenario: Chỉnh sửa từ màn hình xem trước
      Given kế toán đang xem trước chứng từ
      And phát hiện thông tin cần sửa
      When kế toán chọn "Quay lại chỉnh sửa"
      Then hệ thống quay về màn hình nhập liệu
      And giữ nguyên dữ liệu đã nhập

    @preview @compare
    Scenario: So sánh với chứng từ tháng trước
      Given kế toán đang xem trước chứng từ tháng 12/2025
      And đã có chứng từ tháng 11/2025
      When kế toán chọn "So sánh với tháng trước"
      Then hệ thống hiển thị bảng so sánh:
        | Thông tin              | Tháng 11/2025 | Tháng 12/2025 | Chênh lệch |
        | Thu nhập chịu thuế     | 30,000,000    | 30,000,000    | 0          |
        | Bảo hiểm               | 3,150,000     | 3,150,000     | 0          |
        | Thu nhập tính thuế     | 26,850,000    | 25,850,000    | -1,000,000 |
        | Số thuế                | 3,827,500     | 3,677,500     | -150,000   |
      And làm nổi bật các thông tin có sự chênh lệch

    @preview @multi-month
    Scenario: Xem trước chứng từ nhiều tháng
      Given kế toán đã nhập thông tin thuế từ tháng 1 đến tháng 12/2025
      When kế toán chọn "Xem trước"
      Then hệ thống hiển thị tổng quan:
        | Thông tin                  | Giá trị       |
        | Kỳ tính thuế               | 1/2025-12/2025|
        | Tổng thu nhập chịu thuế    | 360,000,000   |
        | Tổng số thuế               | 45,930,000    |
      And có thể xem chi tiết từng tháng

    @preview @confirm
    Scenario: Phát hành từ màn hình xem trước
      Given kế toán đang xem trước chứng từ
      And đã kiểm tra thông tin chính xác
      When kế toán chọn "Phát hành"
      Then hệ thống xác nhận "Bạn có chắc muốn phát hành chứng từ này?"
      When kế toán xác nhận phát hành
      Then chứng từ được sinh số và gửi lên CQT
