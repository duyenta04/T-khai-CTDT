...
Feature: Tạo mới tờ khai đăng ký/thay đổi thông tin sử dụng chứng từ điện tử
  ...
  Rule: Xác định loại tờ khai
    ...
    @type-selection
    Scenario: Mặc định chọn "Đăng ký mới" khi chưa có thông tin đăng ký thành công
      Given MST "0101234567" chưa có tờ khai nào ở trạng thái "CQT chấp nhận"
      And các tờ khai trước đó (nếu có) đều ở trạng thái "CQT từ chối"
      When người dùng nhấn nút "Lập tờ khai"
      Then hệ thống tự động gán Loại tờ khai là "Đăng ký mới"
      And không cho phép người dùng thay đổi sang "Thay đổi thông tin"

    @type-selection
    Scenario: Cho phép linh hoạt chọn loại tờ khai khi hệ thống chưa có dữ liệu thành công
      Given MST "0101234567" chưa có tờ khai nào ở trạng thái "CQT chấp nhận" trên hệ thống hiện tại
      When người nộp thuế thực hiện "Lập tờ khai"
      Then hệ thống mặc định hiển thị loại tờ khai là "Đăng ký mới"
      And hệ thống vẫn cho phép người dùng chủ động chọn sang "Thay đổi thông tin"
    
    # Ghi chú nghiệp vụ cho Dev:
    # 1. Chọn "Đăng ký mới": Dành cho NNT lần đầu dùng chứng từ điện tử.
    # 2. Chọn "Thay đổi thông tin": Dành cho NNT đã được CQT chấp nhận ở phần mềm khác 
    #    nay chuyển sang phần mềm này (Hệ thống nội bộ chưa có dữ liệu nên vẫn phải mở option này).

    @type-selection
    Scenario: Mặc định chọn "Thay đổi thông tin" khi đã có đăng ký
      Given MST "0101234567" đã có ít nhất 01 tờ khai ở trạng thái "CQT chấp nhận"
      When người nộp thuế thực hiện "Lập tờ khai"
      Then hệ thống tự động gán Loại tờ khai là "Thay đổi thông tin"
      And Hệ thống cập nhật thông tin dựa trên tờ khai đã được chấp nhận gần nhất
  ...
  Rule: Giá trị mặc định khi tạo tờ khai Đăng ký mới
    ...
    Scenario: Mặc định chọn "Tổ chức, cá nhân phát hành" ở Đối tượng phát hành khi tạo tờ khai Đăng ký mới
      Given MST "0101234567" chưa có tờ khai nào ở trạng thái "CQT chấp nhận"
      When người nộp thuế tạo tờ khai Đăng ký mới
      Then hệ thống tự động tích chọn "Tổ chức, cá nhân phát hành" ở mục Đối tượng phát hành
      And "Cơ quan thuế phát hành" không được tích chọn

    @default-values @new-registration
    Scenario: Mặc định chọn "Chứng từ điện tử khấu trừ thuế thu nhập cá nhân" ở Loại hình sử dụng khi tạo tờ khai Đăng ký mới
      Given MST "0101234567" chưa có tờ khai nào ở trạng thái "CQT chấp nhận"
      When người nộp thuế tạo tờ khai Đăng ký mới
      Then hệ thống tự động tích chọn "Chứng từ điện tử khấu trừ thuế thu nhập cá nhân" ở mục Loại hình sử dụng
      And các loại hình sử dụng khác không được tích chọn

    @default-values @new-registration
    Scenario: Mặc định chọn "Thông qua tổ chức cung cấp dịch vụ hóa đơn điện tử" ở Hình thức gửi dữ liệu khi tạo tờ khai Đăng ký mới
      Given MST "0101234567" chưa có tờ khai nào ở trạng thái "CQT chấp nhận"
      When người nộp thuế tạo tờ khai Đăng ký mới
      Then hệ thống tự động tích chọn "Thông qua tổ chức cung cấp dịch vụ hóa đơn điện tử" ở mục Hình thức gửi dữ liệu
      And các hình thức gửi dữ liệu khác không được tích chọn

    @default-values @new-registration
    Scenario: Hiển thị đầy đủ các giá trị mặc định khi tạo tờ khai Đăng ký mới
      Given MST "0101234567" chưa có tờ khai nào ở trạng thái "CQT chấp nhận"
      When người nộp thuế tạo tờ khai Đăng ký mới
      Then hệ thống tự động tích chọn các giá trị mặc định
      ...
      And người nộp thuế có thể thay đổi các giá trị này trước khi nộp tờ khai
  ...
  Rule: Loại hình sử dụng chứng từ điện tử
    ...
    Scenario: Hiển thị các loại hình sử dụng có sẵn
      When người nộp thuế tạo tờ khai mới
      Then hệ thống hiển thị các hình sử dụng
    ...
    Scenario: Hiển thị các loại biên lai con khi chọn "Biên lai điện tử" ở loại hình sử dụng
      Given người nộp thuế đang tạo tờ khai
    ...
    @document-type @conditional
    Scenario: Ẩn các loại biên lai con khi bỏ chọn "Biên lai điện tử" ở loại hình sử dụng
  ...
  Rule: Thông tin chữ ký số
      #Áp dụng tương tự với module hóa đơn điện tử
